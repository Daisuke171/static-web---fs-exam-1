...
const container = document.getElementById("favorites-container-actors");
...
(async function renderFavActors() {
  const container = document.getElementById("favorites-container-actors");
  if (!container) return;

  container.innerHTML = "";

  const favIds = JSON.parse(localStorage.getItem("favoritesActors")) || [];
  

  if (favIds.length === 0) {
    container.innerHTML = "<p style='color: white;'>No hay actores favoritos.</p>";
    return;
  }

  const actors = await fetchFavoriteActors(favIds);
